...
int insert(struct node_t **node, void *key, void *value, int (*ucmp)(void *, void *)) 
{
	...
	tmp = *node; //NODE MUST BE A NULL POINTER TO INSERT B/C ITS ATTACHING IT.  RECURSIVELY CALLS THE FUNCTION UNTIL SUCCESS
	...
	comp = (*ucmp)(key, tmp->key); //COMPARES WITH CURRENT NODE ASSUMING IT WAS INSERTED.  IF COMPARISON IS THE SAME THEN WILL RETURN FAILURE
...
int bst_insert(T bst, void *key, void *value, int (*ucmp)(void *, void *))
{
	...
	if (insert(&bst->head, key, value, ucmp) == 0) {  //USER PROVIDES THE OPAQUE POINTER AND THEN THIS FUNCTION GIVES IT TO THE ACTUAL INSERT FUNCTION
		bst->count++;
		return 0;
	}
	else {
