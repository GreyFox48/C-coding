 ...
 * @brief Reads in a dictionary and can find a word or best play from characters
 * @details the dictionary must be newline seperated.  Will tell if the words exits
...
int main(void)
{
        ...
        while (1) {
                ...
                case 1: /* find word option */
                        ...
                        input[strcspn(input, "\n")] = 0;
                        word_temp = bsearch(input, dict->word, dict->nval, sizeof(char *), strcompare);
                        if (word_temp == NULL) {
...
/**
 * Compares two words to each other. Utilized for bsearch in main.
 * @param str1 the word to search for in the dictionary
 * @param str2 the word to compare to str1
 * @return the difference between str1 and str2
 */
static int strcompare(const void *str1, const void *str2)
{
        return strcmp((char *) str1, * (char * const *) str2);
}
